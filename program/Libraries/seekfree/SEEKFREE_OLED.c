...
void oled_init(void)
{
	gpio_init (OLED_SCL_PIN, GPO,1,NOPULL);
    gpio_init (OLED_SDA_PIN, GPO,0,PULLUP);
    gpio_init (OLED_RST_PIN, GPO,1,NOPULL);
    gpio_init (OLED_DC_PIN , GPO,1,NOPULL);
    gpio_init (OLED_CS_PIN , GPO,1,NOPULL);
...
//  Sample usage:			
//-------------------------------------------------------------------------------------------------------------------
void oled_p6x8str(uint8 x,uint8 y, const unsigned char ch[],uint8 color)
{
	uint8 c=0,i=0,j=0;
	if(color==0)
		while (ch[j]!='\0')
...
void oled_p6x8str_and_ch(uint8 x,uint8 y, const unsigned char str[], uint8 digit, unsigned char ch,uint8 color)
{
	uint8 c=0,i=0,j=0;
	digit=x+digit*6;
	if(color==0)
		while (str[j]!='\0')
		{
				...
				if(x!=digit)
				  for(i=0;i<6;i++)	oled_wrdat(oled_6x8[c][i]);
					...
					for(i=0;i<6;i++)	oled_wrdat((oled_6x8[c][i])|(oled_6x8[ch-32][i]));
				x+=6;
				j++;
		}
	else
...
//-------------------------------------------------------------------------------------------------------------------
void oled_p8x16str(uint8 x,uint8 y,uint8 ch[],uint8 color)
{
	uint8 c=0,i=0,j=0;
	if(color==0)
		while (ch[j]!='\0')
...
//  Sample usage:			
//-------------------------------------------------------------------------------------------------------------------
void oled_uint16(uint8 x, uint8 y, uint16 num,uint8 color)
{
	uint8 ch[7];
	
	oled_hexascii(num,ch);
    
    oled_p6x8str(x, y, &ch[1],color);	    //显示数字  6*8字体
...
//  Sample usage:			
//-------------------------------------------------------------------------------------------------------------------
void oled_int16(uint8 x, uint8 y, int16 num,uint8 color)
{
	uint8_t ch[7];
	if(num<0)   {num = -num;oled_p6x8str(x, y, (uint8 *)"-",color);}
	else         oled_p6x8str(x, y, (uint8 *)" ",color);
	x+=6;       
...
//-------------------------------------------------------------------------------------------------------------------
void oled_printf_int32(uint16 x,uint16 y,int32 dat,uint8 num,uint8 color)
{
    int8    buff[34];
    uint8   length;
    ...
    oled_p6x8str(x, y, (uint8 *)buff,color);	//显示数字
}
...
//                          负数会显示一个 ‘-’号   正数显示一个空格
//-------------------------------------------------------------------------------------------------------------------
void oled_printf_float(uint16 x,uint16 y,double dat,uint8 num,uint8 pointnum,uint8 color)
{
    uint8   length;
	int8    buff[34];
...
void oled_printf_float_and_ch(uint16 x,uint16 y,double dat,uint8 num,uint8 pointnum,int8 digit,unsigned char ch,uint8 color)
{
    uint8   length;
	int8    buff[34];
	int8    start,end,point;
    ...
    buff[end] = '\0';
    oled_p6x8str_and_ch(x, y, (uint8 *)buff,digit,ch,color);	//显示数字
}
