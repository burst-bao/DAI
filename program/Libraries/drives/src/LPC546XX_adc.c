...
uint16 adc_convert(ADCCH_enum ch, ADCRES_enum resolution)
{
    ...
    {
        adc_delay(35);//延时大约1us 180M的主频
        delay_num++;
        if(delay_num>=10)
        {
            
            SYSCON->AHBCLKCTRLCLR[0] = SYSCON_AHBCLKCTRL_ADC0_MASK;     //关闭ADC时钟
            SYSCON->PRESETCTRLSET[0] = SYSCON_PRESETCTRL_ADC0_RST_MASK; //复位ADC时钟
            SYSCON->PDRUNCFGCLR[0] = ( 0
                             //| SYSCON_PDRUNCFGCLR_PDEN_ADC0_MASK 
                             | SYSCON_PDRUNCFGCLR_PDEN_VD2_ANA_MASK 
                             | SYSCON_PDRUNCFGCLR_PDEN_VDDA_MASK
            ...
            adc_init(ch);
            return 0;//在自己的程序中发现ADC返回0，那么就需要注意了可能这个0是因为这里 直接返回的而不是有效值
        }
    }
