...
void ctimer_pwmmux(CTIMER_PWMCH_enum pwmch)
{
    switch(pwmch/8)
    {
        ...
        case TIMER3:
        {
            ...
            else if (TIMER3_PWMCH2_A27 == pwmch)  iocon_init(A27,ALT3 | DIGITAL | FILTEROFF | NOPULL);
...
void ctimer_countmux(CTIMER_COUNTCH_enum pwmch)
{
    switch(pwmch/8)
    {
        case TIMER0:
        {
            ...
            else if (TIMER0_COUNT0_A13 == pwmch)  iocon_init(A13,ALT3 | DIGITAL | FILTEROFF | GPIOMODE);
            else if (TIMER0_COUNT1_A2  == pwmch)  iocon_init(A2 ,ALT2 | DIGITAL | FILTEROFF | PULLUP);
            else if (TIMER0_COUNT1_A14 == pwmch)  iocon_init(A14,ALT3 | DIGITAL | FILTEROFF | GPIOMODE);
            else if (TIMER0_COUNT2_B28 == pwmch)  iocon_init(B28,ALT3 | DIGITAL | FILTEROFF | PULLUP);
...
void ctimer_pwm_duty(CTIMER_PWMCH_enum pwmch, uint32 duty)
{
    ...
    switch(ctimer_num)
    {
        ...
        case 1: ASSERT(TIMER1_PWM_DUTY_MAX>=duty);  break;
        case 2: ASSERT(TIMER2_PWM_DUTY_MAX>=duty);  break;
        case 3: ASSERT(TIMER3_PWM_DUTY_MAX>=duty);  break;
        case 4: ASSERT(TIMER4_PWM_DUTY_MAX>=duty);  break;
    }
...
//  @param      countch     计数通道号及引脚
//  @return     void      
//  Sample usage:           ctimer_count_clean(TIMER0_COUNT0_A1);     
//-------------------------------------------------------------------------------------------------------------------
